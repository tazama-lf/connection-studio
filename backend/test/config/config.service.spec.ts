...
describe('ConfigService', () => {
  ...
  it('exports config successfully', async () => {
    mockRepo.findConfigById.mockResolvedValue({ id: 1, status: ConfigStatus.APPROVED });
  ...
  const result = await service.getConfigById(
    1,
    'tenant_001',
    token,
  );
...
it('submits configuration for approval and sends notification', async () => {
  ...
  mockSftp.createFile.mockRejectedValue(
  ...
  await expect(
    ...
    'Failed to export config: SFTP write failed',
  );
});
  ...
  it('throws error for invalid workflow action', async () => {
    await expect(
      service.handleWorkflowAction(1, 'invalid', {}, user as any, token),
    ).rejects.toThrow(BadRequestException);
  });
...
it('throws BadRequestException if SFTP read fails during deploy', async () => {
  ...
  it('updates publishing status', async () => {
    ...
    mockRepo.createDeployedConfig.mockRejectedValue(
      new Error('Insert failed'),
    );

    await expect(
      service.handleWorkflowAction(1, 'deploy', {}, user as any, token),
    ...
    await service.handleWorkflowAction(1, 'deploy', {}, user as any, token);

    expect(
      mockRepo.createTazamaDataModelTable,
  ...
  await expect(
    service.handleWorkflowAction(1, 'export', {}, user as any, token),
...
it('throws BadRequestException when notifyDems fails', async () => {
  ...
  mockNotify.notifyDems.mockRejectedValue(
    new Error('NATS down'),
  );
...
it('removes mapping via service', async () => {
  ...
  const result = await service.removeFunctionViaService(
    1,
    0,
    token,
  );

  expect(mockRepo.removeFunction).toHaveBeenCalledWith(
    1,
    0,
    token,
  );
...
it('removes function via service', async () => {
  ...
  const result = await service.removeMappingViaService(
    1,
    0,
    token,
  );

  expect(mockRepo.removeMapping).toHaveBeenCalledWith(
    1,
    0,
    token,
  );
...
it('adds mapping via service', async () => {
  ...
  expect(mockRepo.addMapping).toHaveBeenCalledWith(
    1,
    mappingData,
    token,
  );
...
it('updates config via write', async () => {
  ...
  const result = await service.updateConfigViaWrite(
    1,
    updateData,
    token,
  );
...
it('adds function via service', async () => {
  ...
  expect(mockRepo.addFunction).toHaveBeenCalledWith(
    1,
    functionData,
    token,
  );
