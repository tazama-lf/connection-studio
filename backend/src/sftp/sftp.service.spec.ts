...
describe('SftpService', () => {
    ...
    describe('createFile', () => {
        ...
        it('should create a file with integrity hash', async () => {
            ...
            jest
                .spyOn(service as any, 'computeSHA256')
        ...
        it('should throw BadRequestException if not in dev environment', async () => {
            ...
            await expect(
                service.createFile(mockFileName, mockData),
            ).rejects.toThrow(BadRequestException);
            await expect(
                service.createFile(mockFileName, mockData),
            ).rejects.toThrow('Exported status can only be set in the dev environment');
        });
        ...
        it('should throw BadRequestException if SFTP host is not configured', async () => {
            ...
            await expect(
                service.createFile(mockFileName, mockData),
            ).rejects.toThrow('Consumer SFTP server credentials not provided');
        });
    ...
    describe('createFileForPublisher', () => {
        ...
        it('should create a file for publisher with integrity hash', async () => {
            ...
            expect(loggerService.log).toHaveBeenCalledWith(
                expect.stringContaining('File uploaded to producer SFTP for publishers'),
            );
        });
    ...
    describe('listFiles', () => {
        const mockFiles = [
            { name: 'de_tenant-123_file1.json', size: 100, modifyTime: 0, accessTime: 0, rights: { user: '', group: '', other: '' }, owner: 0, group: 0, type: '-' as const },
            { name: 'de_tenant-123_file2.json', size: 200, modifyTime: 0, accessTime: 0, rights: { user: '', group: '', other: '' }, owner: 0, group: 0, type: '-' as const },
            { name: 'de_tenant-456_file3.json', size: 300, modifyTime: 0, accessTime: 0, rights: { user: '', group: '', other: '' }, owner: 0, group: 0, type: '-' as const },
            { name: 'cron_tenant-123_file4.json', size: 400, modifyTime: 0, accessTime: 0, rights: { user: '', group: '', other: '' }, owner: 0, group: 0, type: '-' as const },
        ];
