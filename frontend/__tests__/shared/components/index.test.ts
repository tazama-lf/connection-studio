...
describe('shared/components/index.ts', () => {
  it('re-exports shared components from barrel', () => {
    ...
    expect(SharedComponents.DeploymentConfirmation).toBe(
      DeploymentConfirmationMock,
    );
    expect(SharedComponents.DataEnrichmentFormModal).toBe(
      DataEnrichmentFormModalMock,
    );
    expect(SharedComponents.JsonDataModal).toBe(JsonDataModalMock);
    expect(SharedComponents.EndpointHistoryModal).toBe(
      EndpointHistoryModalMock,
    );
