...
describe('CronJobConfirmationDialog', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render export confirmation dialog', () => {
      ...
      expect(
        screen.getByText('Export Confirmation Required!'),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Are you sure you want to export/i),
      ).toBeInTheDocument();
      expect(screen.getByText('"test-export-job"')).toBeInTheDocument();
      expect(
        screen.getByText(
          /⚠️ Important: This will update the cron job status to EXPORTED/i,
        ),
      ).toBeInTheDocument();
    });
    ...
    it('should render approval confirmation dialog', () => {
      ...
      expect(
        screen.getByText('Approval Confirmation Required!'),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Are you sure you want to submit for approval/i),
      ).toBeInTheDocument();
      expect(screen.getByText('"test-approval-job"')).toBeInTheDocument();
      expect(
        screen.getByText(
          /⚠️ Important: This will submit the cron job for approval and update its status to UNDER REVIEW/i,
        ),
      ).toBeInTheDocument();
    });
    ...
    it('should render approve confirmation dialog', () => {
      ...
      expect(
        screen.getByText(
          /⚠️ Important: This will update the cron job status to APPROVED/i,
        ),
      ).toBeInTheDocument();
    });
    ...
    it('should display correct button text for export type', () => {
      ...
      expect(
        screen.getByRole('button', { name: /Yes, Export Cron Job/i }),
      ).toBeInTheDocument();
    });
  ...
  describe('User Interactions', () => {
    ...
    it('should call onConfirm with "approval" when approval confirm button is clicked', () => {
      ...
      const confirmButton = screen.getByRole('button', {
        name: /Yes, Submit for Approval/i,
      });
      fireEvent.click(confirmButton);
  ...
  describe('Edge Cases', () => {
    ...
    it('should display all text elements for approval', () => {
      ...
      expect(
        screen.getAllByText(/submit for approval/i).length,
      ).toBeGreaterThan(0);
