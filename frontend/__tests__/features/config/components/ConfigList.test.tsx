...
jest.mock('../../../../src/features/auth/contexts/AuthContext', () => ({
  useAuth: () => ({
    user: { claims: ['editor', 'exporter', 'approver', 'publisher'] },
  }),
}));
...
jest.mock('../../../../src/features/config/services/configApi', () => ({
  configApi: {
    ...
    updatePublishingStatus: (...args: any[]) =>
      updatePublishingStatusMock(...args),
  },
}));
...
const statusRows = [
  ...
  },
  {
    ...
    msgFam: 'S10',
    endpointPath: '/s10',
    status: 'STATUS_10_CHANGES_REQUESTED',
    createdAt: '2025-01-01',
  },
  {
    id: 21,
...
describe('features/config/components/ConfigList.tsx', () => {
  ...
  it('loads configs and triggers row actions', async () => {
    ...
    await waitFor(() => {
      expect(updateConfigStatusMock).toHaveBeenCalledWith(
        1,
        'STATUS_02_ON_HOLD',
      );
    });
    ...
    await waitFor(() => {
      expect(exportConfigMock).toHaveBeenCalledWith(
        3,
  ...
  it('uses onConfigSelect fallback, renders filters/headers, and updates pagination', async () => {
    ...
    render(<ConfigList onConfigSelect={onConfigSelect} />);
  ...
  it('covers additional status badge branches and status update error handlers', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('rows-count')).toHaveTextContent(
        String(statusRows.length),
      );
    });
    ...
    await waitFor(() => {
      expect(showErrorMock).toHaveBeenCalledWith(
        'Error',
        'Failed to publish config. Please try again.',
      );
    });
    ...
    await waitFor(() => {
      expect(showErrorMock).toHaveBeenCalledWith(
        'Error',
        'Failed to export config. Please try again.',
      );
    });
  ...
  it.each([
    ...
    'renders dialog content for %s confirmation type',
    (type, heading, ctaText) => {
      const actualUseState = React.useState;
      const useStateSpy = jest
        .spyOn(React, 'useState')
        .mockImplementationOnce(() => ['', jest.fn()] as any)
        ...
        .mockImplementationOnce(
          () => [{ open: true, type, config: rows[0] }, jest.fn()] as any,
        )
        .mockImplementation((initial: any) => actualUseState(initial));
  ...
  it('renders action loading text in confirm dialog', () => {
    ...
    const useStateSpy = jest
      ...
      .mockImplementationOnce(
        () =>
          [{ open: true, type: 'export', config: rows[0] }, jest.fn()] as any,
      )
      .mockImplementation((initial: any) => actualUseState(initial));
  ...
  it('Cancel button in confirm dialog calls setConfirmDialog to close (line 752)', () => {
    ...
    const useStateSpy = jest
      ...
      .mockImplementationOnce(
        () =>
          [
            { open: true, type: 'export', config: rows[0] },
            setConfirmDialog,
          ] as any,
      )
      .mockImplementation((initial: any) => actualUseState(initial));
  ...
  it('handleClickOutside false branch when showStatusFilter is false', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText('Loading configurations...'),
  ...
  it.each([
    ...
    [
      'deactivate',
      'Yes, Deactivate Configuration',
      () => updatePublishingStatusMock,
      320,
    ],
  ] as const)(
    'if(onRefresh) false branch - %s without onRefresh (line %s)',
    async (type, ctaText, getMock) => {
      const actualUseState = React.useState;
      const useStateSpy = jest
        .spyOn(React, 'useState')
        .mockImplementationOnce(() => ['', jest.fn()] as any)
        ...
        .mockImplementationOnce(() => [{}, jest.fn()] as any)
        .mockImplementationOnce(
          () => [{ open: true, type, config: rows[2] }, jest.fn()] as any,
        )
        .mockImplementation((initial: any) => actualUseState(initial));
      render(
        <ConfigList
  ...
  it('getDemsStatusLov uses ?? [] fallback when userRole is unknown', async () => {
    const roleUtilsMock = jest.requireMock(
      '../../../../src/utils/common/roleUtils',
    );
    const spy = jest
      .spyOn(roleUtilsMock, 'getPrimaryRole')
      .mockReturnValue('unknown_role');
    render(<ConfigList />);
    await waitFor(() => {
      expect(screen.getByTestId('rows-count')).toBeInTheDocument();
      expect(
  ...
  it.each([
    ...
    'confirm handler does nothing when confirmDialog.config is null (%s)',
    (type, ctaText) => {
      const actualUseState = React.useState;
      const useStateSpy = jest
        .spyOn(React, 'useState')
        ...
        .mockImplementationOnce(() => [{}, jest.fn()] as any)
        .mockImplementationOnce(
          () => [{ open: true, type, config: null }, jest.fn()] as any,
        )
        .mockImplementation((initial: any) => actualUseState(initial));
      render(<ConfigList />);
      fireEvent.click(screen.getByText(ctaText));
  ...
  it('renders correctly when user has no claims', async () => {
    const AuthContextMock = jest.requireMock(
      '../../../../src/features/auth/contexts/AuthContext',
    );
    const spy = jest
      .spyOn(AuthContextMock, 'useAuth')
