...
jest.mock('lucide-react', () => ({
  ClockIcon: ({ size }: { size?: number }) => (
    <svg data-testid="clock-icon" data-size={size} />
  ),
  ListIcon: ({ size }: { size?: number }) => (
    <svg data-testid="list-icon" data-size={size} />
  ),
}));
...
describe('features/config/services/index.tsx', () => {
  it('renders create and manage tab buttons', () => {
    ...
    expect(
      screen.getByRole('button', { name: /manage jobs/i }),
    ).toBeInTheDocument();
  ...
  it('highlights active create tab and switches when manage is clicked', () => {
    ...
    render(
      <CronTabNavigation activeTab="create" setActiveTab={setActiveTab} />,
    );
