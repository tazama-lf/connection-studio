...
jest.mock('@mui/material/Alert', () => (props: any) => <div data-testid="mui-alert">{props.children}</div>);
...
describe('features/data-enrichment/components/PushConfigForm/index.tsx', () => {
  it('renders fields, preview block, and validation errors', () => {
    ...
    expect(screen.getByTestId('EndpointNameInputField-name')).toBeInTheDocument();
    expect(screen.getByTestId('VersionInputField-version')).toBeInTheDocument();
    expect(screen.getByTestId('ApiPathInputField-endpointPath')).toBeInTheDocument();
    expect(screen.getByTestId('DatabaseTableInputField-targetTable')).toBeInTheDocument();
    expect(screen.getByTestId('SelectField-ingestMode')).toHaveAttribute('data-options-len', '1');
    expect(screen.getByText('Endpoint Path Preview')).toBeInTheDocument();
    expect(screen.getByText('Example: /tenantId/enrichment/v1.0.0/customer/data')).toBeInTheDocument();
  ...
  it('falls back to empty ingest options when schema options are missing', () => {
    ...
    jest.isolateModules(() => {
      PushConfigFormDynamic = require('../../../../../src/features/data-enrichment/components/PushConfigForm').default;
    });
    ...
    expect(screen.queryByTestId('validation-name-error')).not.toBeInTheDocument();
  });
});