...
jest.mock('@mui/material/Grid', () => (props: any) => <div data-testid="mui-grid">{props.children}</div>);
jest.mock('@mui/material/Box', () => (props: any) => <div data-testid="mui-box">{props.children}</div>);

jest.mock(
  '../../../../../src/shared/components/FormFields',
  () => {
    const makeField = (label: string) => (props: any) => (
      <div
        data-testid={`${label}-${props.name}`}
        data-options-len={Array.isArray(props.options) ? props.options.length : -1}
      >
        {label}:{props.name}
      </div>
    );
...
jest.mock('../../../../../src/shared/components/ValidationError', () => (props: any) => (
...
jest.mock('../../../../../src/features/data-enrichment/components/validationSchema', () => ({
  ...
  getAssociatedScheduleOptions: (...args: any[]) => getAssociatedScheduleOptionsMock(...args),
}));

import PullConfigForm from '../../../../../src/features/data-enrichment/components/PullConfigForm';

const buildWatch = (values: Record<string, any>) => (field: string) => values[field];

describe('features/data-enrichment/components/PullConfigForm/index.tsx', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    getAssociatedScheduleOptionsMock.mockReturnValue([{ label: 'Sched A', value: 'A' }]);
  });
  ...
  it('renders SFTP key + csv branches and validation errors', () => {
    render(
      <PullConfigForm
        control={{} as any}
        watch={buildWatch({ sourceType: 'sftp', authType: 'key', fileFormat: 'csv' }) as any}
    ...
    expect(screen.getByTestId('MultiLineTextInputField-password')).toBeInTheDocument();
    expect(screen.queryByTestId('PasswordInputField-password')).not.toBeInTheDocument();
    expect(screen.getByTestId('DelimiterInputField-delimiter')).toBeInTheDocument();
    expect(screen.getByTestId('FilePathInputField-pathPattern')).toBeInTheDocument();
  ...
  it('renders SFTP password + non-csv branches without delimiter', () => {
    render(
      <PullConfigForm
        control={{} as any}
        watch={buildWatch({ sourceType: 'sftp', authType: 'password', fileFormat: 'json' }) as any}
        availableSchedules={[] as any}
        errors={{} as any}
      />
    );

    expect(screen.getByTestId('PasswordInputField-password')).toBeInTheDocument();
    expect(screen.queryByTestId('MultiLineTextInputField-password')).not.toBeInTheDocument();
    expect(screen.queryByTestId('DelimiterInputField-delimiter')).not.toBeInTheDocument();
  });
  ...
  it('renders HTTP branch and falls back to empty schedule options', () => {
    ...
    render(
      <PullConfigForm
        control={{} as any}
        watch={buildWatch({ sourceType: 'http', authType: 'password', fileFormat: 'csv' }) as any}
    ...
    expect(screen.getByTestId('MultiLineTextInputField-headers')).toBeInTheDocument();
    ...
    expect(screen.getByTestId('SelectField-schedule')).toHaveAttribute('data-options-len', '0');
  ...
  it('covers fallback empty arrays when schema option constants are missing', () => {
    ...
    jest.doMock('../../../../../src/features/data-enrichment/components/validationSchema', () => ({
      getAssociatedScheduleOptions: () => [{ label: 'Sched', value: 'S' }],
    }));

    let PullConfigFormDynamic: any;
    jest.isolateModules(() => {
      PullConfigFormDynamic = require('../../../../../src/features/data-enrichment/components/PullConfigForm').default;
    });
    ...
    expect(screen.getByTestId('SelectField-authType')).toHaveAttribute('data-options-len', '0');
    expect(screen.getByTestId('SelectField-fileFormat')).toHaveAttribute('data-options-len', '0');
    expect(screen.getByTestId('SelectField-ingestMode')).toHaveAttribute('data-options-len', '0');
  });

  it('covers schema object fallback branch when validation module is undefined', () => {
    jest.resetModules();

    jest.doMock('../../../../../src/features/data-enrichment/components/validationSchema', () => undefined);
