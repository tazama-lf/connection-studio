...
describe('features/data-enrichment/handlers/index.ts', () => {
  ...
  it('scheduleApi endpoints call expected API routes', async () => {
    ...
    expect(urls.some((u: string) => u.includes('/scheduler/create'))).toBe(
      true,
    );
    expect(
      urls.some((u: string) => u.includes('/scheduler/all?offset=5&limit=25')),
    ).toBe(true);
    expect(urls.some((u: string) => u.includes('/scheduler/sched-1'))).toBe(
      true,
    );
    expect(
      urls.some((u: string) => u.includes('/scheduler/update/sched-2')),
    ).toBe(true);
    expect(
      urls.some((u: string) =>
        u.includes(
  ...
  it('determineSourceType chooses from source_type and connection payload', () => {
    ...
    expect(
      determineSourceType({
        connection: JSON.stringify({ host: '10.0.0.1' }),
      } as any),
    ).toBe('SFTP');
    expect(
      determineSourceType({
  ...
  it('updateStatus omits request body when reason is not provided', async () => {
    ...
    expect(apiRequest).toHaveBeenCalledWith(
      expect.stringContaining(
        '/job/update/status/job-10?status=STATUS_04_APPROVED&type=pull',
      ),
  ...
  it('determineSourceType handles object connection payloads', () => {
    expect(
      determineSourceType({ connection: { host: '1.2.3.4' } } as any),
    ).toBe('SFTP');
    expect(
      determineSourceType({ connection: { url: 'https://x.com' } } as any),
    ).toBe('HTTP');
  });
  ...
  it('handleCloneJob validates required fields and clone branches', async () => {
    ...
    expect(showError).toHaveBeenCalledWith(
      'Connector name is required for pull jobs',
    );
  ...
  it('wrapper methods call update/status methods with expected constants', async () => {
    ...
    expect(calls).toContain(
      '/update/activation/job-on?status=active&type=pull',
    );
    expect(calls).toContain(
      '/update/activation/job-off?status=in-active&type=push',
    );
  });
  ...
  it('updateJobData routes to pull or push update APIs', async () => {
    ...
    const urls = (apiRequest as jest.Mock).mock.calls.map((c) => c[0]);
    expect(
      urls.some((u: string) => u.includes('/job/update/job-push?type=push')),
    ).toBe(true);
    expect(
      urls.some((u: string) => u.includes('/job/update/job-pull?type=pull')),
    ).toBe(true);
  });
  ...
  it('saveDataEnrichmentJob creates new push and updates rejected pull', async () => {
    (buildPushPayload as jest.Mock).mockReturnValue({
      endpoint_name: 'push-1',
    });
    (buildPullPayload as jest.Mock).mockReturnValue({
      endpoint_name: 'pull-1',
    });
    (apiRequest as jest.Mock)
    ...
    await saveDataEnrichmentJob({
      ...
      selectedJob: {
        id: 'job-r',
        status: DATA_ENRICHMENT_JOB_STATUSES.REJECTED,
      },
      onSave: jest.fn(),
      onCloseWithRefresh: jest.fn(),
      onClose: jest.fn(),
  ...
  it('prepareJobForEdit and getErrorMessage delegate to utility functions', () => {
    ...
    (getDataEnrichmentErrorMessage as jest.Mock).mockReturnValue(
      'mapped error',
    );
  ...
  it('getJobHistory builds body with searchingFilters and jobId', async () => {
    ...
    await dataEnrichmentJobApi.getJobHistory('job-hist', 2, 20, {
      endpoint_name: 'ep',
    });
  ...
  it('handleInputChange updates edited job via setEditedJob updater', () => {
    ...
    const setEditedJob = jest
      .fn()
      .mockImplementation((updater: (prev: object) => object) => {
  ...
  it('handleSubmitForApproval reports failure when scheduleApi throws', async () => {
    ...
    expect(showError).toHaveBeenCalledWith(
      'Failed to submit schedule for approval',
    );
  });
  ...
  it('handleCloneJob handles null SFTP connection for pull job', async () => {
    ...
    expect(showError).toHaveBeenCalledWith(
      expect.stringContaining('SFTP connection details'),
    );
  ...
  it('handleCloneJob handles null HTTP connection for pull job', async () => {
    ...
    expect(showError).toHaveBeenCalledWith(
      expect.stringContaining('HTTP connection details'),
    );
    expect(setIsCloning).toHaveBeenLastCalledWith(false);
  });
  ...
  it('handleCloneJob uses existing approved schedule when no schedule_id on pull job', async () => {
    (apiRequest as jest.Mock)
      .mockResolvedValueOnce([
        { id: 'found-sched-id', schedule_status: 'approved' },
      ])
  ...
  it('handleCloneJob creates new schedule when no approved schedules exist', async () => {
    (apiRequest as jest.Mock)
      ...
      .mockResolvedValueOnce([
        {
          id: 'fresh-sched',
          name: 'Schedule for new-ep (Cloned)',
  ...
  it('handleCloneJob errors when schedule creation returns success=false', async () => {
    ...
    expect(showError).toHaveBeenCalledWith(
      expect.stringContaining('valid schedule'),
    );
    expect(setIsCloning).toHaveBeenLastCalledWith(false);
  });
  ...
  it('handleCloneJob handles findOrCreateSchedule throwing an error', async () => {
    ...
    expect(showError).toHaveBeenCalledWith(
      expect.stringContaining('Failed to create or find a schedule'),
    );
    expect(setIsCloning).toHaveBeenLastCalledWith(false);
  });
  ...
  it('dataEnrichmentJobApi.getList with status in filters skips default status (line 65 false branch)', async () => {
    (apiRequest as jest.Mock).mockResolvedValue({ data: [], total: 0 });
    await dataEnrichmentJobApi.getList(
  ...
  it('determineSourceType falls through when connectionObj has neither host nor url (line 414/417 false)', () => {
    expect(
      determineSourceType({
        connection: JSON.stringify({ other: true }),
      } as any),
    ).toBe('HTTP');
    expect(determineSourceType({ connection: { other: true } } as any)).toBe(
      'HTTP',
    );
  });
  ...
  it('createPullJobData includes file only when job.file is set (line 514)', async () => {
    (apiRequest as jest.Mock).mockResolvedValueOnce({ id: 'no-file-cloned' });
    const onSuccess = jest.fn();
    await handleCloneJob({
      job: {
        id: 'pull-nofile',
  ...
  it('handleUpdateJobStatus uses statuslabel fallback when status not in map (line 673)', async () => {
    ...
    expect(showSuccess).toHaveBeenCalledWith(
      'Job status updated to CUSTOM_STATUS',
    );
  });
  ...
  it('determineSourceType returns HTTP when connection parses to a non-object', () => {
    ...
    expect(determineSourceType({ connection: '"a string"' } as any)).toBe(
      'HTTP',
    );
  });
