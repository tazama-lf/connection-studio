...
jest.mock('@mui/material/styles', () => ({
  styled:
    ...
    (styleFn: any) => {
      ...
      styleCalls.push(styleFn({ theme: { spacing: (v: number) => `${v * 8}px` } }));
    ...
    jest.mock('@mui/x-data-grid', () => ({ DataGrid: (_props: any) => React.createElement('div', null) }));
...
describe('common/Tables/CustomTable/Table.styles.ts', () => {
  ...
  it('covers shouldForwardProp and conditional style objects', () => {
    const dataGridOptions = styledOptions.find((opt) => opt && opt.shouldForwardProp);
    ...
    const multiline = styleFn({ multilineHeader: true, horizontalScroll: false, horizontalScrollTextAlign: 'left' });
    const horizontal = styleFn({ multilineHeader: false, horizontalScroll: true, horizontalScrollTextAlign: 'right' });
