  ...
  const loadConfigDetails = async () => {
    ...
    try {
      ...
      setSimulationResults({
        input: { transactionId: '12345', amount: 100.5, currency: 'USD' },
        output: { id: '12345', value: 100.5, currencyCode: 'USD' },
        status: 'success',
      });
    } catch (error) {
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    {configDetails.transactionType}
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">
                      Event Type:
                    </span>{' '}
                    {configDetails.msgFam ?? 'N/A'}
                  </div>
                  <div>
              ...
              <div className="border rounded-lg p-4">
                ...
                <div className="bg-gray-50 p-3 rounded font-mono text-sm">
                  <div className="text-green-600">
                    POST {configDetails.endpointPath}
                  </div>
                  <div className="text-gray-600 mt-1">
                    Content-Type:{' '}
                    {configDetails.contentType ?? 'application/json'}
                  </div>
                </div>
              </div>
              ...
              {configDetails.mapping && configDetails.mapping.length > 0 && (
                <div className="border rounded-lg p-4">
                  <div className="flex items-center mb-3">
                    ...
                    <h4 className="text-md font-medium text-gray-900">
                      Field Mappings ({configDetails.mapping.length})
                    </h4>
                  </div>
                  <div className="space-y-2">
                    {configDetails.mapping.map(
              ...
              {/* Functions */}
              {configDetails.functions &&
                configDetails.functions.length > 0 && (
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center mb-3">
                      <EyeIcon className="h-5 w-5 text-orange-500 mr-2" />
                      <h4 className="text-md font-medium text-gray-900">
                        Runtime Functions ({configDetails.functions.length})
                      </h4>
                    </div>
                    <div className="space-y-2">
                      {configDetails.functions.map(
