...
export const EndpointHistoryModal: React.FC<EndpointHistoryModalProps> = ({
  ...
  return (
    ...
    >
      ...
      ></div>
      <div
        className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-hidden relative z-10 shadow-2xl"
        data-id="element-1054"
      >
        <div
          className="flex justify-between items-center px-6 py-4 border-b border-gray-200"
          data-id="element-1055"
        >
          <h2
            className="text-xl font-semibold text-gray-800"
            data-id="element-1056"
          >
            Version History
          </h2>
          <button
          ...
          <div className="relative" data-id="element-1060">
            {versionHistory.map((version, index) => (
              <div
                ...
                data-id="element-1061"
              >
                {index < versionHistory.length - 1 && (
                  <div
                    className="absolute left-4 top-8 bottom-0 w-0.5 bg-gray-200"
                    data-id="element-1062"
                  />
                )}
                <div
                  className="relative flex items-start"
                  data-id="element-1063"
                >
                  <div
                    className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 ring-8 ring-white"
                    data-id="element-1064"
                  >
                    <GitCommitIcon
                      size={16}
                      className="text-blue-600"
                      data-id="element-1065"
                    />
                  </div>
                  <div className="ml-4 flex-1" data-id="element-1066">
                    <div
                      className="flex items-center justify-between"
                      data-id="element-1067"
                    >
                      <h3
                        className="text-lg font-medium text-gray-900"
                        data-id="element-1068"
                      >
                        {version.version}
                      </h3>
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${version.status === 'deployed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}
                        data-id="element-1069"
                      >
                        {version.status === 'deployed' && (
                          <CheckCircleIcon
                            size={12}
                            className="mr-1"
                            data-id="element-1070"
                          />
                        )}
