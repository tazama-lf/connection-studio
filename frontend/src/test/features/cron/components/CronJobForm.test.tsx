...
describe('CronJobForm', () => {
  ...
  describe('Cron Expression', () => {
    ...
    it('should render cron expression in large font', () => {
      ...
      // Find the expression in the display area (not the cron picker)
      const expressionElements = screen.getAllByText('0 0 * * *');
      const displayElement = expressionElements.find(el => 
        el.parentElement?.style.fontSize === '32px' || 
        el.parentElement?.className.includes('css-n7fdxh') ||
        el.tagName === 'DIV'
      );
      // The font size is set via MUI sx prop, so we check the className instead
      expect(displayElement?.parentElement?.className).toMatch(/css-19idom/);
    });
