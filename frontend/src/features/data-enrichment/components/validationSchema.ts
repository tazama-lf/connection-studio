...
export const pullValidationSchema = yup.object({
  ...
  delimiter: yup.string().when(['sourceType', 'fileFormat'], {
    is: (sourceType, fileFormat) =>
      sourceType === 'sftp' && fileFormat === 'csv',
    then: (schema) =>
      schema
        .required('Delimiter is required for CSV files')
        .length(
          DELIMITER_EXACT_LENGTH,
          'Delimiter must be exactly 1 character',
        ),
    otherwise: (schema) => schema.nullable(),
  }),
        ...
        function (value) {
          ...
          try {
            ...
            if (
              ...
              return this.createError({
                message:
                  'Invalid JSON format. Try: {"content-type": "application/json"} or {key: value}',
              });
            }
          }
        },
      ...
      schema
        ...
        .test('valid-url', 'Please enter a valid URL', function (value) {
          ...
          try {
            ...
            const ipPattern =
              /^https?:\/\/(?<oct1>\d{1,3})\.(?<oct2>\d{1,3})\.(?<oct3>\d{1,3})\.(?<oct4>\d{1,3})/;
            ...
            if (ipMatch?.groups) {
              ...
              const invalidOctets = octets.filter(
                (octet) => parseInt(octet, 10) > MAX_IP_OCTET,
              );
            ...
            return this.createError({
              message: 'Invalid URL format. Examples: https://api.example.com',
            });
          }
        })
        ...
        .when('fileFormat', (fileFormat, schema) =>
          schema.test(
            ...
            function (value) {
              ...
              if (
                allowedFormats &&
                !allowedFormats.includes(formatValue.toLowerCase())
              ) {
  ...
  host: yup.string().when('sourceType', {
    is: 'sftp',
    then: (schema) => schema.required('Host is required for SFTP connections'),
    otherwise: (schema) => schema.nullable(),
  }),
...
export const pushValidationSchema = yup.object({
  ...
  endpointPath: yup
    ...
    .test('valid-api-path', 'Please enter a valid API path', function (value) {
      ...
      if (
        trimmedValue.length > PATH_ROOT_MIN_LENGTH &&
