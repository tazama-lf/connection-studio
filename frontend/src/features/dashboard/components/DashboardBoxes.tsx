...
import { ActivityIcon, DatabaseIcon, ClockIcon, PackageIcon } from 'lucide-react';
...
const BoxCard: React.FC<{
  ...
  return (
    ...
    >
      {selected && (
        <Box sx={{ position: 'absolute', top: 12, right: 12, width: 28, height: 28, borderRadius: '50%', bgcolor: alpha(color, 0.12), border: `2px solid ${alpha(color, 0.28)}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Box sx={{ width: 10, height: 10, bgcolor: color, borderRadius: '50%' }} />
        </Box>
      )}

      <Box className="box-icon" sx={{ width: 84, height: 84, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: iconBg, border: `1px solid ${iconBorder}`, transition: 'transform 200ms ease, box-shadow 200ms ease, background-color 200ms ease, border-color 200ms ease', boxShadow: '0 10px 30px rgba(14,22,36,0.04)' }}>
        {React.isValidElement(icon) ? React.cloneElement(icon as any, { size: 34, color }) : icon}
      </Box>

      <Box sx={{ mt: 1.5 }}>
        <Typography variant="h6" sx={{ fontWeight: 700, letterSpacing: '-0.01em', mb: 0.5 }}>{title}</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 320 }}>{subtitle}</Typography>
      </Box>
    </Paper>
  );
};
...
const DashboardBoxes: React.FC = () => {
  ...
  if (isPublisher) {
    items.push({ id: 'exported', title: 'Exported Items', subtitle: 'Review exported items ready for publishing (Cron Jobs, DE Jobs, DEMS)', icon: <PackageIcon />, color: '#7c3aed', path: resolvePath('exported') });
  }
  ...
  return (
    <Box sx={(theme: any) => ({ px: { xs: 2, sm: 4, md: 8 }, pt: { xs: 2, sm: 3 }, mb: 2, backgroundColor: theme.palette.background.default, borderRadius: 2, py: 2 })}>
